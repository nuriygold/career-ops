...
- Baseline CV selection is mandatory before creating any application submission documents. Use the following decision workflow:
  ...
  15. Before labeling any JD requirement missing or allowing a role to reach `SKIP`, run an agent-led capability inference pass. Inspect each prior employer's business type, industry, products/services, operating environment, and the candidate's position held; then map likely requirement evidence across roles, not only by literal keyword matches in the CV.
  16. Use business-context mappings as evidence leads that require truthful framing, not as permission to invent scope. Examples: T-Mobile or Ideagen DevonWay may flag consumer/technical product delivery and hardware-adjacent launch work; T-Mobile or Warrior Body Spa may flag packaging, retail, direct-to-consumer, and customer-experience operations; KSW may flag industrial-design and property/product planning exposure; Wellstar may flag maternal-care and healthcare operations; Harvard and Warrior Body Spa may flag supply-chain, vendor, and contract-manufacturing exposure.
  17. For every high-weight requirement, record the inferred employer/industry/role signal, confidence (`direct`, `context-supported`, `transferable`, or `unresolved`), and the next verification action. `context-supported` requirements remain eligible for tailoring using precise employer-bound language; they are not treated as absent.
  18. A role cannot be skipped solely because a keyword is absent from the current CV. `SKIP` requires a hard disqualifier, a closed/invalid posting, a contradiction, or an unresolved requirement that remains genuinely essential after the capability inference and clarification pass.
...
- **Capability inference pass:** For each high-weight JD requirement, agents must ask: what businesses did the candidate work in, what products/services did those businesses deliver, what operating environment did the role expose her to, and which adjacent employers or projects could evidence the capability? Create an internal mapping such as `requirement -> employer/industry/role signal -> confidence -> wording/verification action`. Promote context-supported signals into the tailored package with bounded wording; do not wait for literal keyword matches. This pass is mandatory before any `SKIP` recommendation.
