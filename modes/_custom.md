...
- Baseline CV selection is mandatory before creating any application submission documents. Use the following decision workflow:
  ...
  12. Distinguish direct evidence from transferable evidence. Use direct language for documented experience; use phrases such as `relevant transferable experience` or `coordinated technical and operational stakeholders` where the JD requirement is adjacent but not directly held. Never convert a transferable capability into a false domain claim.
